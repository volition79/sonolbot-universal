...
- 현재 버전: `0.1.23`
- 현재 zip SHA256: `e5f249e3576c19f88c657fb472dbe97b6ad37d6cd1e6960968e96975aaa1dcfd`
...
Claude Code를 사용할 경우에는 봇을 추가하기 전에 WSL/Ubuntu 터미널에서 `claude`를 직접 실행해 로그인까지 완료하세요.
Claude Code는 npm이 아닌 방식으로 설치되어 있을 수도 있으므로, npm 설치 여부보다 터미널에서 `claude`가 정상 실행되고 로그인된 상태인지가 중요합니다.
로그인 완료 후 `4.클로드코드로그인.bat`를 실행하고, 그 다음 컨트롤패널에서 provider를 `claude-code`로 선택해 봇을 추가하세요.
...
Claude Code를 사용할 경우에는 봇을 추가하기 전에 터미널에서 `claude`를 직접 실행해 로그인까지 완료하세요.
로그인 완료 후 `./4.클로드코드로그인.command`를 실행하고, 그 다음 컨트롤패널에서 provider를 `claude-code`로 선택해 봇을 추가하세요.
...
처음 실행 직후 Claude Code 또는 Codex의 `현재 버전`, `최신 버전`이 비어 있거나 `업데이트하기` 버튼이 보이지 않을 수 있습니다.
이 경우 상단의 `새로고침` 버튼을 누른 뒤 30~60초 정도 기다리세요.
최초 설치 직후에는 provider 버전 정보가 캐시에 없어 비어 보일 수 있으며, 새로고침 후 표시되는 것이 정상입니다.
새로고침 후에도 Claude Code 정보가 계속 비어 있으면 터미널에서 `claude` 실행 및 로그인 완료 여부를 먼저 확인하세요.
